...
const JobFilterStatic: React.FC = () => {
  ...
  return (
    <div className="w-full  flex items-start justify-center">
      <Card className="w-full max-w-md bg-white p-4 rounded-lg">
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            {specialties.map((specialty, index) => (
              <div
                key={index}
                className="flex items-center px-3 py-1 text-xs font-medium text-black rounded-full"
              >
                {specialty}
                <button
                  onClick={() => handleRemoveSpecialty(specialty)}
                  className="ml-2 text-xs text-black hover:text-gray-300 focus:outline-none"
                >
                  ✕
                </button>
              </div>
